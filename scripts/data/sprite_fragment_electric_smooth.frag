...
void main() {
    ...
    // Smooth kaleidoscope using smooth functions instead of hard abs
    float slices = 6.0;
    float pi = 3.14159265359;
    float dist = length(p);
    float ang = atan(p.y, p.x);
    
    // Smooth mirror instead of hard mod
    float sectorSize = pi * 2.0 / slices;
    float sector = ang / sectorSize;
    sector = sector - floor(sector);
    
    // Smooth triangle wave instead of abs
    ...
    // Map to UV and wrap
    vec2 texUV = smoothWrap(kaleidoPos * 0.6 / vec2(aspect, 1.0) + 0.5);
