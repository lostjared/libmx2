...
public:
    ...
    void load(gl::GLWindow *win) override {
        ...
        if (!frame.empty()) {
            ...
            const unsigned char fallback[4] = {255, 0, 255, 255};
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1,
                        0, GL_RGBA, GL_UNSIGNED_BYTE, fallback);
        }
    ...
    void draw(gl::GLWindow *win, const glm::mat4& view, const glm::mat4& projection, const glm::vec3& cameraPos) {
        ...
        glUniformMatrix4fv(glGetUniformLocation(wallShader.id(), "model"), 1, GL_FALSE, glm::value_ptr(model));
        glUniformMatrix4fv(glGetUniformLocation(wallShader.id(), "view"), 1, GL_FALSE, glm::value_ptr(view));
        glUniformMatrix4fv(glGetUniformLocation(wallShader.id(), "projection"), 1, GL_FALSE, glm::value_ptr(projection));

        GLint timeLoc = glGetUniformLocation(wallShader.id(), "time_f");
        if (timeLoc >= 0) glUniform1f(timeLoc, static_cast<float>(SDL_GetTicks()) / 1000.0f);

        GLint resLoc = glGetUniformLocation(wallShader.id(), "iResolution");
        if (resLoc >= 0) glUniform2f(resLoc, win->w, win->h);
        ...
        if (!frame.empty()) {
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame.cols, frame.rows,
                        GL_RGBA, GL_UNSIGNED_BYTE, frame.ptr());
        }

        GLint texLoc = glGetUniformLocation(wallShader.id(), "samp");
    ...
    void updateTexture(void *buffer, int w, int h) {
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, textureId);
...
public:
    ...
    void load(gl::GLWindow *win) override {
        cap.open(filename);
        if(!cap.is_open()) {
            throw mx::Exception("MXCapture: could not open: " + filename);
        }
        readFrame(win);
    ...
    void readFrame(gl::GLWindow *win) {
        cv::Mat temp;
        if(!cap.read(temp)) {
        ...
        cv::cvtColor(temp, frame,cv::COLOR_BGR2RGBA);
        cv::flip(frame, frame, -1);
    }

    void draw(gl::GLWindow *win) override {
        readFrame(win);
        glClearColor(0.102f, 0.133f, 0.173f, 1.0f);
        ...
        game_walls.draw(win, view, projection, game_floor.getCameraPosition());    
        glCullFace(GL_BACK);
