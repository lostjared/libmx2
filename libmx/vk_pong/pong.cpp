...
public:
    ...
    void createGradientPipeline() {
        ...
        auto vertShaderCode = mx::readFile(util.getFilePath("gradient_vert.spv"));
        auto fragShaderCode = mx::readFile(util.getFilePath("gradient_frag.spv"));
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &gradientPipelineLayout) != VK_SUCCESS) {
            throw mx::Exception("Failed to create gradient pipeline layout!");
        }
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &gradientPipeline) != VK_SUCCESS) {
            throw mx::Exception("Failed to create gradient graphics pipeline!");
        }
        
        vkDestroyShaderModule(device, fragShaderModule, nullptr);
        vkDestroyShaderModule(device, vertShaderModule, nullptr);
        
        std::cout << ">> [GradientPipeline] Created for background\n";
    }
    ...
    void draw() override {
        ...
        vkCmdBindPipeline(commandBuffers[imageIndex], VK_PIPELINE_BIND_POINT_GRAPHICS, gradientPipeline);
