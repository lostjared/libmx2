...
namespace mx {
    ...
    void VKWindow::initVulkan() {
        ...
        try {
            textRenderer.reset(new VKText(device, physicalDevice, graphicsQueue, commandPool, util.getFilePath("data/keifont.ttf"), 24));
    ...
    void VKWindow::createVertexBuffer() {
        ...
        std::string modelPath = util.getFilePath("data/geo.mxmod");
        std::cout << ">> Loading MXMOD from: " << modelPath << std::endl;
        loadMXMOD(modelPath, vertices, indices, 10.0f);
        std::cout << ">> After loadMXMOD: " << vertices.size() << " verts, " << indices.size() << " indices" << std::endl;
        if (!vertices.empty()) {
            std::cout << ">> First vertex pos: (" << vertices[0].pos[0] << ", " << vertices[0].pos[1] << ", " << vertices[0].pos[2] << ")" << std::endl;
            std::cout << ">> First vertex texCoord: (" << vertices[0].texCoord[0] << ", " << vertices[0].texCoord[1] << ")" << std::endl;
        }
        ...
        std::cout << ">> [VertexBuffer] Loaded MXMOD with " << vertices.size() << " vertices, " << indices.size() << " indices\n";
    }
