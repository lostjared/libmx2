...
namespace mx {
    ...
    void VKWindow::cleanupSwapChain() {
        ...
        if (graphicsPipelineStarfield != VK_NULL_HANDLE) {
            vkDestroyPipeline(device, graphicsPipelineStarfield, nullptr);
            graphicsPipelineStarfield = VK_NULL_HANDLE;
        }
    ...
    void VKWindow::createGraphicsPipeline() {
        try {
            ...
            if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipelineStarfield) != VK_SUCCESS) {
                throw mx::Exception("Failed to create starfield graphics pipeline!");
            }
    ...
    void VKWindow::draw() {
        ...
        VkPipeline pipelineToUse = (currentPolygonMode == VK_POLYGON_MODE_LINE) ? graphicsPipelineStarfield : graphicsPipeline;
        ...
        (void)indexCount; (void)vertexBuffer; (void)indexBuffer;
    ...
    void VKWindow::createTextPipeline() {
        try {
            std::vector<char> textVertShaderCode;
            std::vector<char> textFragShaderCode;
            try {
                textVertShaderCode = readFile(util.getFilePath("data/text_vert.spv"));
                textFragShaderCode = readFile(util.getFilePath("data/text_frag.spv"));
            } catch (const std::exception& e) {
                SDL_Log("Text shaders not found under 'data/': %s -- trying top-level assets path", e.what());
                textVertShaderCode = readFile(util.getFilePath("text_vert.spv"));
                textFragShaderCode = readFile(util.getFilePath("text_frag.spv"));
            }
