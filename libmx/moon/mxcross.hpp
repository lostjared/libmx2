...
namespace cross {
    ...
    inline void loadMXMOD(
        ...
        if (vcount <= 0)
            throw mx::Exception("loadMXMOD: invalid vert count");

        std::vector<Vec3> pos(vcount);
        std::string line;
        int read = 0;
        while (read < vcount) {
            if (!readNonCommentLine(line))
        ...
        if (tcount != vcount)
            throw mx::Exception("loadMXMOD: tex count != vert count");

        std::vector<Vec2> uv(vcount);
        read = 0;
        while (read < vcount) {
            if (!readNonCommentLine(line))
                throw mx::Exception("loadMXMOD: failed reading texcoords");
            std::istringstream iss(line);
            if (!(iss >> uv[read].x >> uv[read].y))
                throw mx::Exception("loadMXMOD: failed reading texcoords");
            ++read;
        }
        ...
        if (ncount != vcount)
            throw mx::Exception("loadMXMOD: norm count != vert count");

        std::vector<Vec3> nrm(vcount);
        read = 0;
        while (read < vcount) {
            if (!readNonCommentLine(line))
                throw mx::Exception("loadMXMOD: failed reading normals");
            std::istringstream iss(line);
            if (!(iss >> nrm[read].x >> nrm[read].y >> nrm[read].z))
                throw mx::Exception("loadMXMOD: failed reading normals");
            ++read;
        }
    ...
    class GL_Window : public WindowBase, public gl::GLWindow {
      public:
        ...
        void init(const std::string &title, const std::string &path, int w, int h, bool full) override {
            ...
            if (full)
                gl::GLWindow::setFullScreen(true);
            if (handler)
                handler->load();
        }
