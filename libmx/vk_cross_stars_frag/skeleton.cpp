...
float magnitudeToSize(float magnitude) {
    return glm::clamp(35.0f - magnitude * 3.0f, 2.0f, 35.0f);
}

float magnitudeToAlpha(float magnitude, float lightPollution) {
    float alpha = (6.5f - magnitude) / 6.5f;
    return glm::clamp(alpha - lightPollution, 0.9f, 1.0f);
}
...
public:
    ...
    void init(const std::string& dataPath) {
        ...
        std::string texPath2 = dataPath + "/data/star.png";
    ...
    void update(float deltaTime) {
        ...
        for (GLuint i = 0; i < NUM_STARS; ++i) {
            ...
            if (star.z > 10.0f) {
                float theta = generateRandomFloat(0.0f, 2.0f * M_PI);
                star.z = generateRandomFloat(-300.0f, -100.0f);
                float r = sqrt(generateRandomFloat(0.0f, 1.0f)) * starFieldRadius * 0.3f;
                star.x = r * cos(theta);
                star.y = r * sin(theta);
            }
...
public:
    ...
    void draw(VkCommandBuffer commandBuffer, uint32_t imageIndex, int screenW, int screenH) {
        ...
        ubo.params = glm::vec4(SDL_GetTicks() / 1000.0f, static_cast<float>(screenW), static_cast<float>(screenH), 1.0f);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        window->init("Cross-Platform Starfield 3D", args.path, args.width, args.height, args.fullscreen);
        window->loop();
        window->cleanup();
