  ...
  public:
    ...
    void createExplosionParticles(Explosion &explosion) {
        ...
        for (int wave = 0; wave < WAVE_COUNT; wave++) {
            ...
            if (spawned >= particlesPerWave * WAVE_COUNT)
                break;
        }
        int total = 0;
        for (auto &p : particles) {
...
class Projectiles {
  ...
  public:
    ...
    ~Projectiles() {
        ...
        if (projectileVBO[0] != 0)
            glDeleteBuffers(3, projectileVBO);
        if (texture != 0)
            glDeleteTextures(1, &texture);
    }
...
class StarFighter {
  public:
    ...
    void load(gl::GLWindow *win) {
        ...
        model.setTexturesFromMTL(win, win->util.getFilePath("data"));
...
class Game : public gl::GLObject {
  ...
  public:
    ...
    void handleInput(gl::GLWindow *win, float deltaTime) {
        ...
        if (controller.getButton(SDL_CONTROLLER_BUTTON_LEFTSHOULDER) || controller.getButton(SDL_CONTROLLER_BUTTON_DPAD_UP)) {
            ship.increaseSpeed(deltaTime);
        } else if (controller.getButton(SDL_CONTROLLER_BUTTON_RIGHTSHOULDER) || controller.getButton(SDL_CONTROLLER_BUTTON_DPAD_DOWN)) {
