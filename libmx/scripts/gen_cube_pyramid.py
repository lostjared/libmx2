...
def add_pyramid_on_face(verts, uvs, norms, center, n, half, height, uv_mode):
    t, b = face_basis(n)
...
def generate_cube_with_pyramids(cube_size, pyramid_height, face_scale):
    verts = []
    uvs = []
    norms = []

    h = cube_size * 0.5
    face_half = h * face_scale
    ...
    for center, n in faces:
        add_pyramid_on_face(verts, uvs, norms, center=center, n=n, half=face_half, height=pyramid_height, uv_mode="face")
...
def main():
    ...
    ap.add_argument("-o", "--out", default="cube_pyramids.mxmod")
    ap.add_argument("--cube_size", type=float, default=200.0)
    ap.add_argument("--pyramid_height", type=float, default=80.0)
    ...
    verts, uvs, norms = generate_cube_with_pyramids(
