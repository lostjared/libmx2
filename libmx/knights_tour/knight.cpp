...
public:
    ...
    virtual void draw(mx::mxWindow *win) override {
        ...
        win->text.printText_Blended(the_font, 15, 5, "Knights Tour - Tap Space, Press Return to Reset");
        win->text.printText_Blended(the_font, 400, 5, "Moves: " + std::to_string(tour.getMoves()));
    }
...
void Tour::drawBoard(mx::mxWindow* win) {
    ...
    for (int i = 0; i < BOARD_SIZE; ++i) {
        for (int j = 0; j < BOARD_SIZE; ++j) {
            SDL_Rect rect = {startX + j * 55, startY + i * 55, 50, 50};
            if (board[i][j] == -1) {
                SDL_SetRenderDrawColor(win->renderer, 0, 0, 0, 255);
            } else if ((i + j) % 2 == 0) {
...
void Tour::drawKnight(mx::mxWindow* win, mx::Texture& tex) {
    SDL_Rect dst = {startX + knightPos.col * 55 + 5, startY + knightPos.row * 55 + 5, 35, 35};
    SDL_RenderCopy(win->renderer, tex.handle(), nullptr, &dst);
}

class MainWindow : public mx::mxWindow {
public:
    MainWindow(const std::string &path) : mx::mxWindow("Knights Tour", 640, 480, false) {
        setPath(path);
