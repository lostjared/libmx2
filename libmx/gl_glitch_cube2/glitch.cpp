...
vec4 waterX(vec2 tc, sampler2D samp) {
    float speed = 5.0;
...
vec4 warp(vec2 tc, sampler2D samp) {
    ...
    vec4 fcolor = texture(samp, warpedCoords);
    return fcolor;
}
...
vec4 alphaXor(vec4 color) {
    ...
    float time_mod = mod(time_f, 20.0); 
...
void main() {
    FragColor = vec4(vertexColor, 1.0) * texture(texture1, TexCoords);
    vec4 ctx = distort4(TexCoords, texture1);
    vec4 wtx = waterX(TexCoords, texture1);
    vec4 wtx2 = warp(TexCoords, texture1);
    FragColor = mix(ctx, wtx, 0.5);
    FragColor = alphaXor(FragColor);
    FragColor.a = 1.0;
}
)";
#endif
...
public:
    ...
    virtual void load(gl::GLWindow *win) override {     
        ...
        if(cube.openModel(win->util.getFilePath("data/star.mxmod")) == false) {
        ...
        cube.setTextures(win, win->util.getFilePath("data/star.tex"), win->util.getFilePath("data"));
    }
