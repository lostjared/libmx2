...
namespace mx {
    ...
    void Mesh::compressIndices() {
        ...
        if (!norm.empty())
            newNorms.reserve(uniqueVertices.size() * 3);
        if (!tex.empty())
            newTex.reserve(uniqueVertices.size() * 2);
    ...
    void Mesh::draw() {
        glBindVertexArray(VAO);
        if (!indices.empty())
            glDrawElements(shape_type, (GLsizei)indices.size(), GL_UNSIGNED_INT, 0);
        else
            glDrawArrays(shape_type, 0, (GLsizei)(vert.size() / 3));
        glBindVertexArray(0);
    }
    ...
    void Mesh::generateTangentBitangent() {
        ...
        auto addTri = [&](GLuint i0, GLuint i1, GLuint i2) {
            ...
            bitangent[p0] += bitan.x;
            bitangent[p0 + 1] += bitan.y;
            bitangent[p0 + 2] += bitan.z;
            bitangent[p1] += bitan.x;
            bitangent[p1 + 1] += bitan.y;
            bitangent[p1 + 2] += bitan.z;
            bitangent[p2] += bitan.x;
            bitangent[p2 + 1] += bitan.y;
            bitangent[p2 + 2] += bitan.z;
        };
    ...
    void Mesh::wave(DeformAxis axis, float amplitude, float frequency, float phase) {
        for (size_t i = 0; i < vert.size(); i += 3) {
            float x = vert[i], y = vert[i + 1];
            if (axis == DeformAxis::Y)
                vert[i + 1] += amplitude * std::sin(x * frequency + phase);
            else if (axis == DeformAxis::X)
                vert[i] += amplitude * std::sin(y * frequency + phase);
            else if (axis == DeformAxis::Z)
                vert[i + 2] += amplitude * std::sin(y * frequency + phase);
        }
    }
    ...
    void Mesh::applyHSV(float h, float s, float v) {
        (void)h;
        (void)s;
        (void)v;
    }
    ...
    void Model::drawArrays() {
        if (!program)
            throw mx::Exception("Shader program must be set in Model before drawArrays");
    ...
    void Model::drawArraysWithTexture(GLuint texture, const std::string texture_name) {
        if (!program)
            throw mx::Exception("Shader program must be set in Model before drawArraysWithTexture");
        for (auto &mesh : meshes)
    ...
    void Model::drawArraysWithCubemap(GLuint cubemap, const std::string texture_name) {
        if (!program)
            throw mx::Exception("Shader program must be set in Model before drawArraysWithCubemap");
        for (auto &mesh : meshes) {
    ...
    void Model::setTextures(const std::vector<GLuint> &textures) {
        ...
        // Build material-name -> texture-id map from loaded MTL materials
        std::unordered_map<std::string, GLuint> matTexMap;
        for (size_t i = 0; i < materials_.size() && i < textures.size(); ++i)
            matTexMap[materials_[i].name] = textures[i];

        for (size_t i = 0; i < meshes.size(); ++i) {
            // Prefer material-name lookup
            if (!meshes[i].materialName.empty()) {
                auto it = matTexMap.find(meshes[i].materialName);
                if (it != matTexMap.end()) {
                    meshes[i].texture = it->second;
                    continue;
                }
            }
            // Fallback: clamp to last texture
            meshes[i].texture = textures.back();
        }
    }
    ...
    void Model::setTexturesFromMTL(gl::GLWindow *win, const std::string &objDir) {
        ...
        std::vector<GLuint> text;
        std::unordered_map<std::string, GLuint> materialTextureMap;
        ...
        for (size_t i = 0; i < materials_.size(); ++i) {
            ...
            if (texid == 0) {
                ...
                glBindTexture(GL_TEXTURE_2D, 0);
                mx::system_out << "mx: MTL placeholder [" << i << "] " << texid
                               << " Kd=(" << materials_[i].kd[0] << "," << materials_[i].kd[1] << "," << materials_[i].kd[2] << ")\n";
            }

            text.push_back(texid);
            materialTextureMap[materials_[i].name] = texid;
        }

        for (auto &mesh : meshes) {
            if (!mesh.materialName.empty()) {
                auto it = materialTextureMap.find(mesh.materialName);
                if (it != materialTextureMap.end()) {
                    mesh.texture = it->second;
                    continue;
                }
            }
            // No material name match — use last texture as fallback
            if (!text.empty())
                mesh.texture = text.back();
        }
