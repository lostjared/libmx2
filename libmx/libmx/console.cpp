...
namespace console {
    ...
    void Console::updateInputScrollOffset() {
        ...
        for (char c : promptText) {
            ...
            if (c == '\t') {
                ...
                charWidth = (it != c_chars.characters.end()) ? it->second->w : c_chars.characters['A']->w;
            }
    ...
    void Console::checkScroll() {
        ...
        while (static_cast<int>(lines.size()) > maxScrollbackLines && !lines.empty()) {
            ...
            if (removeLen > buffer.length())
                removeLen = buffer.length();
    ...
    SDL_Surface *Console::drawText() {
        ...
        if (!surface)
            return nullptr;
        if (!needsRedraw && fadeState == FADE_NONE && surface)
            return surface;
    ...
    bool Console::isMouseOverScrollbar(int mouseX, int mouseY) const {
        bool hit = mouseX >= scrollbarRect.x && mouseX < scrollbarRect.x + scrollbarRect.w && mouseY >= scrollbarRect.y && mouseY < scrollbarRect.y + scrollbarRect.h;
        return hit;
    }
