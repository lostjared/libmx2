...
// Smooth wrapping function - no derivative discontinuities
vec2 smoothWrap(vec2 uv) {
    return fract(uv);
}
...
void main() {
    ...
    // Small flow offset
    vec2 flow = vec2(f1 - 0.5, f2 - 0.5) * 0.2;
    
    // Map to UV and wrap smoothly
    vec2 finalPos = warped + flow;
    vec2 texUV = smoothWrap(finalPos * 0.5 / vec2(aspect, 1.0) + 0.5);
    
    // Chromatic aberration
    vec2 dir = normalize(p) * 0.004;
    
    float r = texture(samp, smoothWrap(texUV + dir)).r;
    float g = texture(samp, texUV).g;
    float b = texture(samp, smoothWrap(texUV - dir)).b;
    vec3 col = vec3(r, g, b);
