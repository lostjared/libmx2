...
namespace mx {
    ...
    void VKWindow::initVulkan() {
        ...
        VkDeviceSize bufferSize = sizeof(ParticleVertex) * MAX_PARTICLES;
    ...
    void VKWindow::createLogicalDevice() {
        ...
        std::vector<VkExtensionProperties> availableExtensionsLinux(extCountLinux);
        vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &extCountLinux, availableExtensionsLinux.data());

        bool hasExternalMemory = false;
        bool hasExternalMemoryFd = false;
        for (const auto& ext : availableExtensionsLinux) {
    ...
    void VKWindow::cleanup() {
        ...
        if (starfieldInitialized) {
            if (starVertexBufferMapped != nullptr) {
    ...
    float VKWindow::generateRandomFloat(float min, float max) {
    ...
    glm::vec3 VKWindow::getStarColor(float temperature) {
        float r, g, b;
    ...
    float VKWindow::magnitudeToSize(float magnitude) {
        return glm::clamp(15.0f - magnitude * 2.0f, 1.0f, 25.0f);
    }

    float VKWindow::magnitudeToAlpha(float magnitude) {
        float alpha = (6.5f - magnitude) / 6.5f;
        return glm::clamp(alpha - lightPollution, 0.0f, 1.0f);
    }

    void VKWindow::initStarfield(int numStarsParam) {
        ...
        std::cout << ">> [Starfield] Initialized with " << numStars << " stars\n";
    }

    void VKWindow::loadStarTexture() {
        SDL_Surface* starImg = png::LoadPNG(util.getFilePath("star.png").c_str());
        if (!starImg) {
            throw mx::Exception("Failed to load star.png texture!");
        }
        
        VkDeviceSize imageSize = starImg->w * starImg->h * 4;
        VkBuffer stagingBuffer;
        VkDeviceMemory stagingBufferMemory;
        createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        ...
        createImage(starImg->w, starImg->h, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL,
            VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, starTexture, starTextureMemory);
        
        transitionImageLayout(starTexture, VK_FORMAT_R8G8B8A8_UNORM, 
            VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        copyBufferToImage(stagingBuffer, starTexture, starImg->w, starImg->h);
        transitionImageLayout(starTexture, VK_FORMAT_R8G8B8A8_UNORM,
        ...
        std::cout << ">> [Starfield] Star texture loaded\n";
    }

    void VKWindow::createStarDescriptorSetLayout() {
        VkDescriptorSetLayoutBinding samplerLayoutBinding{};
        samplerLayoutBinding.binding = 0;
        samplerLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        samplerLayoutBinding.descriptorCount = 1;
        ...
        std::cout << ">> [Starfield] Descriptor set layout created\n";
    }

    void VKWindow::createStarDescriptorPool() {
        std::array<VkDescriptorPoolSize, 2> poolSizes{};
        poolSizes[0].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        poolSizes[0].descriptorCount = static_cast<uint32_t>(swapChainImages.size());
        poolSizes[1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        ...
        std::cout << ">> [Starfield] Descriptor pool created\n";
    }

    void VKWindow::createStarDescriptorSets() {
        std::vector<VkDescriptorSetLayout> layouts(swapChainImages.size(), starDescriptorSetLayout);
        ...
        starDescriptorSets.resize(swapChainImages.size());
        ...
        std::cout << ">> [Starfield] Descriptor sets created\n";
    }

    void VKWindow::createStarVertexBuffer() {
        VkDeviceSize bufferSize = sizeof(StarVertex) * numStars;
    ...
    void VKWindow::createStarPipeline() {
        try {
            ...
            std::cout << ">> [Starfield] Pipeline created successfully\n";
        } catch (const std::exception& e) {
            std::cerr << "Warning: Star pipeline creation failed: " << e.what() << std::endl;
            std::cerr << "Compile star shaders with:" << std::endl;
            std::cerr << "  glslc star_vertex.vert -o star_vert.spv" << std::endl;
            std::cerr << "  glslc star_fragment.frag -o star_frag.spv" << std::endl;
            starPipeline = VK_NULL_HANDLE;
        }
    }

    void VKWindow::updateStarfield(float deltaTime) {
        if (!starfieldInitialized || !starVertexBufferMapped) return;
        
        if (deltaTime > 0.1f) deltaTime = 0.1f;
        
        float time = SDL_GetTicks() * 0.001f;
        
        StarVertex* vertices = static_cast<StarVertex*>(starVertexBufferMapped);
    ...
    void VKWindow::drawStarfield(VkCommandBuffer cmdBuffer, uint32_t imageIndex) {
        if (!starfieldInitialized || starPipeline == VK_NULL_HANDLE) return;
        
        Uint32 currentTime = SDL_GetTicks();
        float deltaTime = (currentTime - lastStarUpdateTime) / 1000.0f;
        ...
        float aspect = static_cast<float>(swapChainExtent.width) / static_cast<float>(swapChainExtent.height);
        ubo.proj = glm::perspective(glm::radians(60.0f), aspect, 0.1f, 1000.0f);
        ubo.proj[1][1] *= -1; 
        ...
        vkCmdDraw(cmdBuffer, numStars, 1, 0, 0);
    }
    
    void VKWindow::createParticlePipeline() {
        try {
            auto vertShaderCode = mx::readFile(util.getFilePath("particle_vert.spv"));
            auto fragShaderCode = mx::readFile(util.getFilePath("particle_frag.spv"));
            ...
            VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &particlePipelineLayout));
            ...
            std::cout << ">> [Particles] Pipeline created successfully\n";
        } catch (const std::exception& e) {
            std::cerr << "Warning: Particle pipeline creation failed: " << e.what() << std::endl;
            std::cerr << "Compile particle shaders with:" << std::endl;
            std::cerr << "  glslc particle_vertex.vert -o particle_vert.spv" << std::endl;
            std::cerr << "  glslc particle_fragment.frag -o particle_frag.spv" << std::endl;
            particlePipeline = VK_NULL_HANDLE;
        }
    }
    
    void VKWindow::drawParticles(VkCommandBuffer cmdBuffer, uint32_t imageIndex) {
        if (particlePipeline == VK_NULL_HANDLE || activeParticleCount == 0) {
            return;
        }
        ...
        if (!descriptorSets.empty()) {
            vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                particlePipelineLayout, 0, 1, &descriptorSets[imageIndex], 0, nullptr);
        }
        
        vkCmdDraw(cmdBuffer, activeParticleCount, 1, 0, 0);
    }
}