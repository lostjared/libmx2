...
struct ComputePC {
    int32_t mode;           // 0 = blur, 1 = blend
    int32_t historyCount;   // valid history frames
    int32_t historyIdx;     // which history frame to blend with
    int32_t square_size;    // block size for spatial blend variation
    int32_t history_dir;    // direction scalar
};
...
private:
    ...
    void initComputeResources() {
...

        {
            ...
            for (auto &img : histImg) allocCImg(img, cmd);
    ...
    void buildComputePipeline() {
        const std::string spvPath = util.path + "/data/compute.spv";
    ...
    void tickAnimState() {
        ...
        if (square_dir == 1) {
            current_square += 2;
            if (current_square >= 64) { current_square = 64; square_dir = 0; }
        } else {
            current_square -= 2;
            if (current_square <= 2)  { current_square = 2;  square_dir = 1; }
        }
    }
    ...
    void destroyComputeResources() {
        ...
        for (auto &img : histImg) destroyCImg(img);
        destroyCImg(outImg);

        if (computeSampler)  vkDestroySampler            (device, computeSampler,  nullptr);
        if (compDSPool)      vkDestroyDescriptorPool     (device, compDSPool,      nullptr);
        if (compPipeline)    vkDestroyPipeline           (device, compPipeline,    nullptr);
        if (compPipeLayout)  vkDestroyPipelineLayout     (device, compPipeLayout,  nullptr);
        if (compDSLayout)    vkDestroyDescriptorSetLayout(device, compDSLayout,    nullptr);
        if (stagingBuf)      vkDestroyBuffer             (device, stagingBuf,      nullptr);
        if (stagingMem)      vkFreeMemory                (device, stagingMem,      nullptr);
        if (readbackBuf)     vkDestroyBuffer             (device, readbackBuf,     nullptr);
